...
// Default tolerance ≈ 5m at equator — visually indistinguishable on mobile
const DEFAULT_TOLERANCE = 0.00005;

// Threshold: only run simplification when point count exceeds this
...
export default memo(function PolylineWrapper({
  ...
  const simplified = useMemo(() => {
    ...
    // Skip simplification for short polylines
    if (!simplify || coordinates.length <= SIMPLIFY_THRESHOLD) {
      return coordinates;
    }

    // Reuse previous result if coordinates haven't changed
  ...
  // Custom comparison: skip re-render if coords and style are identical
